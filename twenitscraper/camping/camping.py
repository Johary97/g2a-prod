...
class CampingScrap(object):
    ...
    def scrap_station_link_details(self, date) -> None:
    ...
    def extract_data(self, page:str, url:object) -> list:
        ...
        try:
            ...
            for result in final_results:
                ...
                prix_actuel = result.find('div', class_='best-offer__price-value').text.strip()[:-2] \
                    if result.find('div', class_='best-offer__price-value') else ''
                prix_init = result.find('div', class_="best-offer__price-old").text.strip()[:-2] \
                    if result.find('div', class_="best-offer__price-old") else prix_actuel
                # n_offres, date_debut, date_fin = self.get_link_data(url[0])
                ...
                data['nom'] = name.typologie.replace('\n', ' ')
                data['Nb personnes'] = adulte.typologie.replace('\n', ' ')
                data['localite'] = localite.typologie.replace('\n', ' ')
                data['n_offre'] = ''
...
def clean_campings(src, dest):
    ...
    with open(dest, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['url','web-scrapper-order', 'date_price', 'date_debut', 'date_fin', 'prix_init', 'prix_actuel', 'typologie', 'Nb Personnes','n_offre', 'nom', 'localite', 'date_debut-jour','Nb semaines', 'cle_station', 'nom_station']
...
if __name__=='__main__':
    CampingScrap('Sat', 'data3.csv', "2023/04/01", "2023/04/29")